...
describe("Generate E2E Tests", () => {
	...
	describe.skipIf(!process.env.NEBIUS_API_KEY)("Nebius Provider (Kimi-K2.5 via OpenAI Completions)", () => {
		const llm = getModel("nebius", "moonshotai/Kimi-K2.5");

		it("should complete basic text generation", { retry: 3 }, async () => {
			await basicTextGeneration(llm, { apiKey: process.env.NEBIUS_API_KEY! });
		});

		it("should handle tool calling", { retry: 3 }, async () => {
			await handleToolCall(llm, { apiKey: process.env.NEBIUS_API_KEY! });
		});

		it("should handle streaming", { retry: 3 }, async () => {
			await handleStreaming(llm, { apiKey: process.env.NEBIUS_API_KEY! });
		});

		it("should handle thinking mode", { retry: 3 }, async () => {
			await handleThinking(llm, { apiKey: process.env.NEBIUS_API_KEY! });
		});

		it("should handle multi-turn with thinking and tools", { retry: 3 }, async () => {
			await multiTurn(llm, { apiKey: process.env.NEBIUS_API_KEY! });
		});
	});
